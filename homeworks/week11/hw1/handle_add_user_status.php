<?php
  ...
  if(empty($title)||!isset($is_able_edit_user)||!isset($is_able_add_post)||!isset($is_able_edit_own_comment)||!isset($is_able_edit_all_comment)||!isset($is_able_delete_own_comment)||!isset($is_able_delete_all_comment)) {
    header("Location: ./add_user_status.php");
    die();
  };

  // 根據新增資訊去資料庫新增 user 的資料
  $sql = "INSERT INTO ykf2020_w11_hw1_user_status(title,is_able_add_post,is_able_delete_own_comment,is_able_delete_all_comment,is_able_edit_own_comment,is_able_edit_all_comment,is_able_edit_user) VALUES(?,?,?,?,?,?,?)";
  $stmt = $conn->prepare($sql);
  $stmt->bind_param('siiiiii', $title, $is_able_add_post, $is_able_delete_own_comment, $is_able_delete_all_comment, $is_able_edit_own_comment, $is_able_edit_all_comment,$is_able_edit_user);
  $result = $stmt->execute();
